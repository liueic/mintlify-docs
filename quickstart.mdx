...
<AccordionGroup>
  ...
  <Accordion icon="cog" title="文件存储配置（可选）">
    如果需要文件上传功能，配置S3或MinIO：
...
<AccordionGroup>
  <Accordion icon="docker" title="Docker部署（推荐）">
    ...
    <Check>服务启动后，访问 http://localhost:3000 查看API文档</Check>
  </Accordion>
...
# 查看支持的Agent列表
curl http://localhost:3000/api/v1/upload/agents
```
...
<Note>
**需要帮助？** 查看我们的[完整文档](/essentials/architecture)或加入[社区讨论](https://github.com/your-repo/ai-gateway/discussions)。
</Note>
